...
# ─── BASE EVENT ───────────────────────────────────────────────────────────────
#
# BaseEvent is DOMAIN-OWNED. It carries only the fields that the domain itself
# assigns: what happened (event_type), which schema version (event_version), and
# the business payload fields declared on each concrete subclass.
#
# Store-assigned envelope fields (event_id, stream_id, positions, recorded_at,
# metadata) deliberately live ONLY on StoredEvent below.

class BaseEvent(BaseModel):
    """Domain-owned base for every event.  No store-assigned fields here."""
    event_type: str
    event_version: int = 1

    def to_payload(self) -> dict:
        """Return the business payload dict (excludes event_type / event_version)."""
        d = self.model_dump(mode='json')
        d.pop('event_type', None)
        d.pop('event_version', None)
        return d

    def to_store_dict(self) -> dict:
        """Produce the minimal dict passed to EventStore.append()."""
        return {
...
# ─── STORED EVENT ─────────────────────────────────────────────────────────────
#
# StoredEvent is a DISTINCT type.  It is created by the event store after a
# successful append and is what consumers (projections, agents) work with.
# It is NOT a subclass of BaseEvent — it is a separate envelope model that
# contains all store-assigned columns plus the original event_type / version.

class StoredEvent(BaseModel):
    """Store-assigned envelope returned by EventStore.load_stream / load_all."""
    # ── Store-assigned envelope fields ──────────────────────────────────────
    event_id: UUID                    # assigned by store (PK)
    stream_id: str                    # e.g. "loan-APP-0001"
    stream_position: int              # position within the stream (0-based)
    global_position: int              # monotonic global position across all streams
    recorded_at: datetime             # wall-clock timestamp set by store
    metadata: dict[str, Any] = Field(default_factory=dict)  # correlation_id, causation_id, …
...
class StreamMetadata(BaseModel):
    """Current state of an event stream, stored in the event_streams table."""
    stream_id: str
    aggregate_type: str
    current_version: int
    created_at: datetime
...
# ─── DOMAIN EXCEPTIONS ────────────────────────────────────────────────────────
#
# These are the CANONICAL exception types for the whole system.
# Import from here — do NOT redefine in event_store.py or elsewhere.

class DomainError(Exception):
    """
    Base class for all domain-layer errors.
    Raised when a business rule is violated (e.g. state-machine guard fails).
    This is a DISTINCT exception type, not aliased to any built-in.
    """
...
class OptimisticConcurrencyError(Exception):
    """
    Raised by EventStore.append() when the expected_version does not match
    the current stream version (another writer modified the stream concurrently).

    Fields are DECLARED on the class (not stored only in the message string)
    so callers can inspect stream_id, expected_version, and actual_version
    programmatically without parsing the error message.
    """
    stream_id: str
